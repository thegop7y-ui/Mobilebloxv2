...
namespace roblox {
    namespace addresses {
        ...
        // Funcs
        constexpr std::uintptr_t scriptcontext_resume = 0x18F4760; // "$Script" // F0 B5 03 AF 2D E9 00 0F 81 B0 2D ED 04 8B AC B0 83 46 E0
        constexpr std::uintptr_t rbxspawn = 0x1932D30; // "Spawn function requires 1 argument" // F0 B5 03 AF 2D E9 00 0F 81 B0 2D ED 04 8B AC B0 05 46 D7
        constexpr std::uintptr_t sandboxthreadandsetidentity = 0x18E51E8; // "Unable to create a new thread for %s" // F0 B5 03 AF 2D E9 00 07 92 46 88 46
        constexpr std::uintptr_t rlua_pushinstance = 0x18A2668; // "InvalidInstance"
        constexpr std::uintptr_t pushinstance_registry = 0x18A20B4; // "InvalidInstance"
        static std::uintptr_t pushinstance_registry_rebased = 0x0; // basically rebased pushinst reg, inititiated at funcs::init
        constexpr std::uintptr_t rbx_getthreadcontext = 0x18DE6B4; // printidentity
        constexpr std::uintptr_t rlua_newthread = 0x36CC828; // "unable to create a new thread for %s"
        constexpr std::uintptr_t fireclickdetector = 0x23CF824; // ClickDetectorE vftable + 0xE4 First function call with 3 arguments in the case 1 of the switch.
        constexpr std::uintptr_t fireproximityprompt = 0x23D3E78; // "ProximityPrompt_triggered"
        constexpr std::uintptr_t firetouchinterest = 0x2F44E84; // "new overlap in different world" function call return
        
        // Func Hooks
        constexpr std::uintptr_t startscript = 0x1920834; // "Script Start" // F0 B5 03 AF 2D E9 00 0F B5 B0 83 46 DF F8 58 0C
        constexpr std::uintptr_t ongameleave = 0x131ED10; // "onGameLeaveBegin() SessionReporterState_GameExitRequested placeId:%lld"
        constexpr std::uintptr_t ongameloaded = 0x131EAC8; // "onGameLoaded() SessionReporterState_GameLoaded placeId:%lld"
        constexpr std::uintptr_t jobstart = 0x3A4C48C; // "[FLog::TaskSchedulerRun] JobStart %s"
        constexpr std::uintptr_t jobstop = 0x3A4C4FC; // "[FLog::TaskSchedulerRun] JobStop %s"
        
        // Now in luaconf.h
        /*
        // luau VM
        constexpr std::uintptr_t luau_execute = 0x36DD6D4; // luaD_call
        
        // luaD_* functions
        constexpr std::uintptr_t luaD_throw = 0x36D245C; // lua_error <- luaL_error // B0 B5 02 AF 05 46 0C 20 0C 46 6D
        constexpr std::uintptr_t luaD_rawrunprotected = 0x36D2400; // luaD_pcall
        
        // luaC_* functions
        // lua_newthread -> B0 B5 02 AF 04 46 00 69 D0
        constexpr std::uintptr_t luaC_step = 0x36D3168; // lua_newthread // F0 B5 03 AF 2D E9 00 0F 81 B0 2D ED 02 8B 84 B0 04 69
        constexpr std::uintptr_t luaC_barriertable = 0x36D3C44; // 00 69 43 7D 02
        
        // luaV_* functions
        constexpr std::uintptr_t luaV_gettable = 0x36E7488;
        constexpr std::uintptr_t luaV_settable = 0x36E75A0;
